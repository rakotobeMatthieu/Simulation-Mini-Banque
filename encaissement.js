...
function encaisseCtrl($window,$scope,$http){
    ...
    $scope.valider = function(){
        ...
        $http.get(url+'/encaisser?id='+($window.sessionStorage.getItem("clientIdClient"))+'&valeur='+val+'&date='+date2).success(function(data) {
            if(data.status === 200){
                alert(data.message);
